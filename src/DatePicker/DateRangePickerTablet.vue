<template>
  <v-card class="date-range-picker-tablet elevation-4 ma-0 mx-sm-auto mt-md-2 pt-4">
...
export default {
  ...
  mounted() {
    ...
    if (this.config) {
      ...
      // This is useful in desktop picker, messy here
      //
  ...
  computed: {
    compare: {
      get() {
        return this.p_compare
      },
      set(val) {
        this.p_compare = val
        this.pickerMainIsActive = !this.p_compare
      },
    }, // compare
  }, // computed
...
.date-range-picker-tablet::v-deep {
  max-width: 785px;
  max-height: 100vh;
  // margin-top: 9vh;
  ...
  .picker-main {
    // Border prevents from weird vertical resizing
    // of the whole box when checking "Compare".
    // Same in picker-compare
